...
export function Step1ChooseTemplate({ selectedTemplate, onSelect, onNext, onBack }: Step1Props) {
  ...
  useEffect(() => {
    async function fetchTemplates() {
      try {
        ...
        setError(err instanceof Error ? err.message : t('settings.templates.errorLoad'));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-white">{t('broadcasts.step1Title')}</h2>
      ...
      {templates.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-slate-800 bg-slate-900/50">
          ...
          <p className="text-sm text-slate-400">{t('broadcasts.noTemplatesTitle')}</p>
          <p className="mt-1 text-xs text-slate-500">{t('broadcasts.noTemplatesDesc')}</p>
        </div>
      ) : (
