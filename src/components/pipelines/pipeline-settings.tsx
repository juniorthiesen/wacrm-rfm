...
export function PipelineSettings({
  ...
  async function handleSave() {
    ...
    onPipelinesChanged();
    onStagesChanged();
    toast.success(t("pipelines.pipelineSaved"));
  }
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error(t("pipelines.moveDealsWarning"));
      return;
    }
        ...
        {showDeleteConfirm ? (
          <div className="py-4">
            <div className="flex items-center gap-3 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-400">
                  {t("pipelines.deletePipelineWarning")}
                </p>
              </div>
            </div>
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                {deleting ? t("common.deleting") : t("pipelines.deletePipeline")}
              </Button>
            </div>
          </div>
        ) : (
          <>
            <div className="grid gap-4 py-2">
              ...
              <div className="grid gap-2">
                <Label className="text-slate-300">{t("pipelines.stagesTitle")}</Label>
                <DndContext
                  sensors={sensors}
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {STAGE_COLORS.map((color) => (
                    <button
                      ...
                      aria-label={`${t("pipelines.changeColor")} ${color}`}
                    />
                  ))}
                </div>
            ...
            <DialogFooter className="border-slate-700 bg-slate-900/50">
              ...
              >
                {saving ? t("common.saving") : t("contacts.saveChanges")}
              </Button>
            </DialogFooter>
          </>
        )}
